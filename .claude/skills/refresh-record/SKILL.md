...
- **`out-of-scope`.** Only on the two criteria the provider's own pages can show, from the five in `CONTRIBUTING.md`: criterion 3, it no longer publishes a price without a login, and criterion 4, it no longer documents its platform publicly. Set `criterion` to 3 or 4 and quote what the page shows instead, a login wall, a contact-sales form, an empty docs domain. Never rule on the other three; they need sources this skill may not read. A record with `status: anonymous` is never moved out of scope on criterion 4, which that list waives.
