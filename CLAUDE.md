...
`.claude/skills/refresh-record/SKILL.md` is the procedure for re-reading a record against the provider's pages, and a scheduled cloud routine runs it daily (MR-320). It writes one commit per record to one of two branches. `claude/refresh` carries confirmations, records re-read and found unchanged with their `checkedAt` dates moved, and `.github/workflows/refresh.yml` merges it into `main` on its own after `scripts/refresh-guard.ts` has checked that dates are all that changed; the rule is `src/lib/confirmation.ts`. `claude/refresh-review` carries value and status changes, and a person merges its pull request. The routine confirms and corrects sourced fields, dates what it read, and sets `status` when the provider's own pages announce a shutdown, an acquisition or a rename, or show that criterion 3 or 4 no longer holds. A domain that does not answer changes nothing. It never touches prose, `name`, or the fields the scripts under `scripts/` own, and the review pull request is where a status change is read before it ships.
