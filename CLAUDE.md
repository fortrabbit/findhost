...
- **Absent means unknown.** Four fields are required of a listed record (`id`, `name`, `urls.home`, `category`), and every record carries `addedAt`, the date it entered the register — a fact about the register, not the provider, and the one date git could supply if the deploy container had history; everything else optional. Never guess, never infer, never default to a flattering value.
- **`checkedAt` is a claim.** It means the record was read against the provider's own pages on that date — by an agent, not necessarily a person. Absent renders as nothing; never fall back to the git commit date. The dates a machine reads, the sitemap's `lastmod`, `dateModified`, the feed's `updated`, are the newest date on the record, `checkedAt` or any source's, via `src/lib/modified.ts`, so a field refreshed on its own still tells a crawler to come back.
- **`status` decides where a record appears, and there are three places.** Listed is the default. `discontinued`, `unlisted` and `anonymous` put a record *beside* the register: out of it by default, offered back as a group of its own, page still indexed and searchable. `draft` and `out-of-scope` hide one from the register, every count, every facet, search and the sitemap, while keeping its page — an out-of-scope record must name the numbered criterion it failed. A decision nobody can link to is not a decision, and nothing is ever deleted. `src/lib/providers.ts` is the only place those filters live — load providers through it, never `getCollection` directly.
...
- **Link text names the document, not the claim.** "documented in the [developer docs](…)", "as the company's [blog post](…) describes", "per [The Register's report](…)". A reader should know what opens before clicking; a link on "public API, CLI and MCP server" was read as an announcement and landed on a reference. The claim stays in plain words around the link.
...
`.claude/skills/refresh-record/SKILL.md` is the procedure for re-reading a record against the provider's pages, and a scheduled cloud routine runs it daily (MR-320). It writes one commit per record to one of two branches. `claude/refresh` carries what merges itself: records re-read and found unchanged with their `checkedAt` dates moved, and records whose field values changed where every changed field carries a `sources` entry naming it. `.github/workflows/refresh.yml` merges it into `main` after `scripts/refresh-guard.ts` has checked exactly that; the rule is `src/lib/confirmation.ts`. `claude/refresh-review` carries the judgement — a `status` change, a `name`, a `figure`, a `description`, the heart, a value nothing cites — and a person merges its pull request. Reviewing every corrected price by hand was a queue nobody had time for, and an unread price is worse than a merged one; a host leaving the register is still somebody's reading of an announcement. The routine confirms and corrects sourced fields, dates what it read, and sets `status` when the provider's own pages announce a shutdown, an acquisition or a rename, or show that criterion 3 or 4 no longer holds. A domain that does not answer changes nothing. It never touches prose, `name`, or the fields the scripts under `scripts/` own, and the review pull request is where a status change is read before it ships.
