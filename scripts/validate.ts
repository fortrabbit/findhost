...
/*
 * A pairing is two facets that become a page together —
 * /runtimes/php/regions/germany/. Declared on one side only, because the side
 * that declares it comes first in the path and the reverse must never also
 * exist: two addresses for one set is the duplicate this dataset would be judged
 * on hardest.
 *
 * Mass-generated facet combinations are the doorway-page pattern. What keeps
 * these from being that is a floor, a narrowing ceiling and a hand-written
 * allowlist, and none of the three is worth anything if a pairing can name a
 * facet that does not exist or a value nobody can phrase in English.
 */
const facetOf = new Map(fields.filter((field) => field.facet).map((field) => [field.facet!, field]));
const declaredPairs = new Set<string>();

for (const field of fields) {
  if (!field.pairs?.length) continue;

  if (!field.facet) {
    fail(dictionaryFile, `"${field.id}" declares pairs but is not a facet, so it heads no address`);
    continue;
  }

  for (const pair of field.pairs) {
    const partner = facetOf.get(pair.with);

    if (!partner) {
      fail(dictionaryFile, `"${field.id}" pairs with "${pair.with}", which is not a facet`);
      continue;
    }

    if (partner.id === field.id) {
      fail(dictionaryFile, `"${field.id}" pairs with itself`);
      continue;
    }

    const both = [field.facet, pair.with].sort().join(' + ');
    if (declaredPairs.has(both)) {
      fail(dictionaryFile, `${both} is declared twice — a pairing has one direction, and it is the declaring side`);
    }
    declaredPairs.add(both);

    /*
     * Every value of the partner facet needs the phrase that names it as the
     * second half of a title. A value with none is silently skipped by
     * lib/pairs.ts, which is the right behaviour at build time and the wrong
     * thing to discover from a missing page.
     */
    const unphrased = partner.values
      .filter((value) => !value.noFilter && !value.hidden)
      .filter((value) => !titleWithOf(partner, value));

    if (unphrased.length && !pair.draft) {
      fail(
        dictionaryFile,
        `${both} is built, but ${unphrased.length} of "${partner.id}" has no titleWith, so those combinations cannot name themselves${unphrased.length <= 4 ? `\n    ${unphrased.map((value) => value.id).join(', ')}` : ''}`,
      );
    }
  }
}
...
/*
 * No two combination pages may ship the same sentence.
 *
 * The generated summary is what stops these being doorway pages, and it is also
 * the thing most likely to collapse: pin a second facet and the clauses that
 * varied on a value page stop varying. Fifty-five pages reading identically
 * would be a content farm generated by our own helper, and nothing else here
 * would notice. Checked against what was actually built rather than recomputed,
 * because the question is what shipped.
 */
const distDir = 'dist';
if (existsSync(distDir)) {
  const built = (dir: string, depth = 0): string[] =>
    depth > 4
      ? []
      : readdirSync(dir, { withFileTypes: true }).flatMap((entry) => {
          const path = join(dir, entry.name);
          if (entry.isDirectory()) return built(path, depth + 1);
          return entry.name === 'index.html' && depth === 4 ? [path] : [];
        });

  const partnersOf = new Map(
    fields.filter((field) => field.facet && field.pairs?.length).map((field) => [field.facet!, field.pairs!]),
  );

  const said = new Map<string, string>();

  for (const file of built(distDir)) {
    const [, facet, , partner] = file.split('/');
    if (!partnersOf.get(facet ?? '')?.some((pair) => pair.with === partner)) continue;

    const summary = readFileSync(file, 'utf8').match(/<p class="annotation">(.*?)<\/p>/)?.[1];
    if (!summary) {
      fail(file, 'a combination page with no summary — the sentence is the page');
      continue;
    }

    const twin = said.get(summary);
    if (twin) fail(file, `says exactly what ${twin} says, so one of them is a duplicate of the other`);
    else said.set(summary, file);
  }
}

/*
 * The two gates, asserted rather than assumed. They are the difference between
 * this and a content farm, and a floor quietly lowered to 2 would look like an
 * ordinary edit.
 */
if (pairFloor < 5) fail('src/lib/pairs.ts', `pairFloor is ${pairFloor}: below five a combination page lists nothing`);
if (pairCeiling > 0.8)
  fail('src/lib/pairs.ts', `pairCeiling is ${pairCeiling}: above 0.8 a combination repeats the page it narrows`);
