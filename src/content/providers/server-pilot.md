...
ServerPilot is a [server management service](/category/server-management/) rather than a hosting service. It sells no servers: the customer connects a fresh Ubuntu machine from any cloud provider, and ServerPilot installs and then maintains the stack on it — Nginx and Apache, several PHP versions side by side, MySQL, a firewall, automatic security updates and certificates. Trading since 2012, it is one of the earliest services in this category.
