...
The value of a shell is mostly diagnostic. Deployments run through git or a pipeline on most platforms here, so shell access matters on the day something behaves differently in production than it did anywhere else, and reading a log or running one command settles it. Restricted shells are common on [shared hosting](/category/shared-hosting/) and on [PaaS](/category/paas/), for different reasons: the first is protecting neighbours, the second is protecting a rebuild.
