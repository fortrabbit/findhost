...
A Virtual Private Server is an empty machine with root access. The provider keeps the hypervisor and the network running; everything above that belongs to the customer, which is why most records here answer "you" to [who manages the operating system](/operating-system/) and offer [full SSH access](/shell/). Resources are allotted per instance rather than shared out among accounts, the software stack is a free choice, and a small instance is one of the cheapest ways to run something in [somebody else's data centre](/metal/).
