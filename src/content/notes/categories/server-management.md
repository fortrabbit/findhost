...
That splits the comparison in two. The bill has two parts, a licence or subscription plus whatever the server costs, so an [entry price band](/entry-price/) in this category prices the panel alone. Responsibility splits the same way: the operating system stays with the customer with a panel to make it manageable, which is why "you, with panel" is the usual answer to [who manages the operating system](/os/) here. A panel automates the routine and can patch unattended. It does not take the pager.
