...
Two shapes sit side by side. The classic control panels — [cPanel](/providers/cpanel/), [Plesk](/providers/plesk/) — are licensed per server and run most of the [shared hosting](/categories/shared-hosting/) industry. The developer-facing provisioners — [Laravel Forge](/providers/laravel-forge/), [Ploi](/providers/ploi/), [RunCloud](/providers/runcloud/), [Coolify](/providers/coolify/) — sit closer to a self-hosted [PaaS](/categories/paas/), deploy pipeline included. A few resell the infrastructure as well and return one bill, which makes them hard to tell from a PaaS from outside. Whose name is on the server account is the difference.
