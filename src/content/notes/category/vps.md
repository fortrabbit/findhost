...
The advertised specifications are softer than they look, too. Providers oversubscribe physical hosts to varying degrees, so identical vCPU counts can differ in real throughput depending on the neighbours. Where the work of running a server is not wanted, [PaaS](/category/paas/) hands the operating system back to the provider, and a [management panel](/category/server-management/) keeps the server while automating most of the routine around it.
